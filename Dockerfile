...
RUN apt-get update && \
    apt-get install -y \
    --no-install-recommends cmake libprotobuf23 libprotobuf-dev protobuf-compiler libudev-dev make gcc g++ zlib1g-dev \
    pkg-config \
    build-essential \
    libssl-dev \
    libpq5 \
...
COPY --from=builder /usr/src/app/target/release/mints /app/mints
...
# we must copy necessary runtime files into this stage for ./mints
COPY ./.env /app/.env
